...
// global
float red_flag = 1.0f;
float green_flag = 0.0f;
float blue_flag = 0.0f;
...
int main() {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // render the triangle
        for (int i = 0; i < GRADIENT_LEVEL; i++) {
            glUniform4f(vertexColorLocation, red_flag * i * (1.0f / GRADIENT_LEVEL),
                        green_flag * i * (1.0f / GRADIENT_LEVEL), blue_flag * i * (1.0f / GRADIENT_LEVEL), 1.0f);
...
void processInput(GLFWwindow *window) {
    ...
    if (glfwGetKey(window, GLFW_KEY_1) == GLFW_PRESS){
        red_flag=1.0f;
        green_flag=blue_flag=0.0f;
    }
    if (glfwGetKey(window, GLFW_KEY_2) == GLFW_PRESS){
        green_flag=1.0f;
        red_flag=blue_flag=0.0f;
    }
    if (glfwGetKey(window, GLFW_KEY_3) == GLFW_PRESS){
        blue_flag=1.0f;
        green_flag=red_flag=0.0f;
    }
}
