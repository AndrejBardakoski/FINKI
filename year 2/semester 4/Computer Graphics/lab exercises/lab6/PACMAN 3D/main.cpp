...
int main() {
    ...
    int const NUMBER_OF_PHI_ANGLES = 20;
    int const NUMBER_OF_THETA_ANGLES = 20;
    float phi = 0.0f;
    float theta = -HALF_PI;

    float phi_increment = TWO_PI / NUMBER_OF_PHI_ANGLES;
    float theta_increment = HALF_PI / NUMBER_OF_THETA_ANGLES;
    ...
    phi_increment = PI / NUMBER_OF_PHI_ANGLES;
    theta_increment = HALF_PI / NUMBER_OF_THETA_ANGLES;
    // quarter ball
    for (auto i = 0; i < NUMBER_OF_THETA_ANGLES; ++i) {
        phi = 0.0f;
        for (int j = 0; j <= NUMBER_OF_PHI_ANGLES; j++) {
...
// mouth
    float angle = PI;
    int const NUMBER_OF_ANGLES = 32;
    float angle_increment = PI / NUMBER_OF_ANGLES;
...
//    glBufferData(GL_ARRAY_BUFFER, sizeof(vertices), vertices, GL_STATIC_DRAW);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        glDrawArrays(GL_TRIANGLE_STRIP, 0, 2 * (2 * (NUMBER_OF_THETA_ANGLES) * (NUMBER_OF_PHI_ANGLES + 1)));
        glDrawArrays(GL_TRIANGLE_STRIP,
                     2 * (NUMBER_OF_THETA_ANGLES) * (NUMBER_OF_PHI_ANGLES + 1),
                     2 * (NUMBER_OF_THETA_ANGLES) * (NUMBER_OF_PHI_ANGLES + 1));
        glUniform4f(colorLoc, 0.9f, 0.4f, 0.1f, 1.0f);
        glDrawArrays(GL_TRIANGLE_FAN, 4 * (NUMBER_OF_THETA_ANGLES) * (NUMBER_OF_PHI_ANGLES + 1), NUMBER_OF_ANGLES+2);
        glDrawArrays(GL_TRIANGLE_FAN, 4 * (NUMBER_OF_THETA_ANGLES) * (NUMBER_OF_PHI_ANGLES + 1)+NUMBER_OF_ANGLES+2,NUMBER_OF_ANGLES+2);
