...
int main() {
    ...
    for (auto i = 0; i <= NUMBER_OF_ANGLES; ++i) {
        vertices.push_back(centerX + radius_Small * glm::cos(angle));
        vertices.push_back(radius_Small * glm::sin(angle));
        vertices.push_back(0.1f);

        vertices.push_back(centerX + radius_Big * glm::cos(angle));
...
//    glBufferData(GL_ARRAY_BUFFER, sizeof(vertices), vertices, GL_STATIC_DRAW);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        glDrawArrays(GL_TRIANGLE_STRIP, 0, 2 * (NUMBER_OF_ANGLES + 1));
        glDrawArrays(GL_TRIANGLE_STRIP, 2 * (NUMBER_OF_ANGLES + 1), 2 * (NUMBER_OF_ANGLES + 1));
        glDrawArrays(GL_TRIANGLE_STRIP, 4 * (NUMBER_OF_ANGLES + 1), 2 * (NUMBER_OF_ANGLES + 1));
        glDrawArrays(GL_TRIANGLE_STRIP, 6 * (NUMBER_OF_ANGLES + 1), 2 * (NUMBER_OF_ANGLES + 1));

        glUniform4f(colorLoc, 0.2f, 0.2f, 0.5f, 1.0f);
        glDrawArrays(GL_TRIANGLE_STRIP,8 * (NUMBER_OF_ANGLES + 1),14);
